...
class BaseAgent(Base):
    def act(self, state, epsilon=0.0):
        ...
        if(rand_num < epsilon):#explore by choosing random action
            output_action = np.random.randint(self.num_actions)
        else:                   #exploit by choosing best action
            output_actions = self.forward(state)
...
def get_env():
    ...
    config = {'observation_type': 'tensor', 'agent_speed_range': [-3, -1], 'stochasticity': 0.0, 'width': 50,
